	...
	std::vector<OBJMaterial>& mats)
{
	...
	if (fin)
	{
		...
		std::ifstream fname("Models\\wall.obj");
		ReadOBJ(fname, mats, subsets, vertices, indices);
...
void OBJLoader::ReadOBJ(std::ifstream& fin, std::vector<OBJMaterial>& mats, std::vector<MeshGeometry::Subset>& subsets,
	std::vector<Vertex::PosNormalTexTan>& vertices, std::vector<USHORT>& indices)
{
	...
	while (!fin.eof()) {
		...
		else if (line.find(('vt', 0, 2)))
		{

		}
		else if (line.find(('vn', 0, 2)))
		{

		}
		else if (line.find(('f ', 0, 2)))
		{
